...
const getPeople = state => {
  ...
  const queryString = searchKeys.filter(key => state[key]).reduce((qs, key) => {
...
const fetchPeopleEpic = (action$, { getState }) =>
  action$.ofType('FETCH_PEOPLE').switchMap(() =>
    getPeople(getState())
      .map(r => r.response)
      .map(people => ({
...
const rootEpic = combineEpics(triggerFetchPeopleEpic, fetchPeopleEpic);
